...
1. **Generated multi-profile mode** -- the classic Gentle AI flow. The base SDD conductor is `qa-orchestrator`. Each named profile generates its own `sdd-orchestrator-{name}` plus 16 suffixed SDD/QA phase sub-agents in `opencode.json`, and you switch between them with **Tab**.
...
Open OpenCode and press **Tab** -- your new orchestrator appears alongside `qa-orchestrator`, the default OpenCode SDD conductor.
...
|---|---|---|
| `qa-orchestrator` | Canonical base OpenCode SDD conductor. All `/sdd-*` commands point here by default. | No |
| `gentle-orchestrator` | Legacy base conductor key. Sync migrates it to `qa-orchestrator`. | No; let sync migrate it |
| `sdd-orchestrator` | Legacy base conductor key. Sync migrates it to `qa-orchestrator`. | No; let sync migrate it |
...
- **preserves the current `qa-orchestrator` prompt** during sync so external tools can keep their runtime policy / fallback blocks intact
...
|---|---|
| `qa-orchestrator` | Default profile (your original config) |
...
If you're using an external single-active manager instead, you typically keep working with the base `qa-orchestrator` while the external tool swaps its active model assignments at runtime.
...
The `default` profile (`qa-orchestrator`) can be edited but not deleted -- it always exists when SDD is configured.
...
- **`generated-multi`** -- scan `opencode.json` for `sdd-orchestrator-*`, update shared prompts, regenerate profile orchestrators, preserve model assignments, and keep `qa-orchestrator` as the canonical base conductor
- **`external-single-active`** -- detect external profile files, keep the shared SDD assets current, and preserve the existing `qa-orchestrator` prompt instead of overwriting external runtime extensions
