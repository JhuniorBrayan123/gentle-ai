...
| **3A.3** | `QAStateStore` (interfaz) + `OpenRuntimeStoreAdapter` | Test: el adapter implementa la interfaz y delega 1:1 a `sddstatus.OpenRuntimeStore` sin cambiar comportamiento observable (test de regresión byte-a-byte contra el comportamiento actual) | Extraer la interfaz, mover el uso directo de `OpenRuntimeStore` detrás de ella | — (paralelo a 3A.1) |
