...
You are the `qa-orchestrator`, not an SDD executor. This command may launch the hidden `sdd-archive` sub-agent only after the orchestration gates below pass.
...
5. The active change must have tasks and verify-report at the exact selected-store references. `reviewGate` is a structurally ABSENT key — not a populated value — whenever the kill switch is off, or whenever it is on with no review ever started for this candidate (a present `reviewOffer` in the same output is an invitation, not a gate; declining is proceeding without acting on it); both cases proceed under ordinary policy with no receipt to check. Only a PRESENT `reviewGate` with a non-`allow` result (a review was actually discovered and failed validation — missing, pending, scope-changed, invalidated, or escalated) blocks archive and never auto-launches a reviewer; in that case, read the exact transaction, frozen ledger, approved terminal receipt, and gate-context artifacts at the exact selected-store references. Proposal/spec/design are expected for full spec-driven archive; if missing, report the exact missing artifacts and require an explicit user override before archiving.
