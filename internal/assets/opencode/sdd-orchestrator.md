...
Before delegating to the next phase agent/sub-agent, the immediate upstream artifact required by the `Sub-Agent Context Protocol` must be **gated for this execution**, not merely present from a prior session. This applies to every edge in the `Dependency Graph` — `proposal -> specs`, `specs`/`design -> tasks`, `tasks -> apply`, `apply -> verify`, `verify -> archive` — not only the entry into `qa-apply`.
