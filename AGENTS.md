...
| `qa-evidence` | When producing the reproducible G6 evidence bundle (types, test run, no fixed waits, Screenplay+POM, docs comparison, exact command+result) for a QA case, standalone. | [`skills/qa-evidence/SKILL.md`](skills/qa-evidence/SKILL.md) |
| `smartops-ui` | When implementing a Vue screen or component from a Figma design using the SmartOps UI design system. | [`skills/smartops-ui/SKILL.md`](skills/smartops-ui/SKILL.md) |
