...
test_cc_skills_full() {
    log_test "Claude Code: skills injection (full-gentleman = 16 foundation skills)"
    cleanup_test_env
    ...
    if $BINARY install --agent claude-code --component skills --preset full-gentleman --persona neutral 2>&1; then
        ...
        # is 16 foundation (7 original + 9 qa-orchestrator-v3) + judgment-day
        # = 17 files; the 11 sdd-* phase skills come only from the SDD
        # component (not selected here).
        # #4669: the six contributor workflow skills are selectable, never default.
        assert_file_count "$skills_dir" "SKILL.md" 17 "Full preset (skills alone): 17 skill files"
...
test_cc_skills_ecosystem() {
    log_test "Claude Code: skills injection (ecosystem-only = 16 foundation skills)"
    cleanup_test_env
    ...
    if $BINARY install --agent claude-code --component skills --preset ecosystem-only --persona neutral 2>&1; then
        ...
        # #3554: skills alone no longer pulls sdd. 16 foundation (7 original +
        # 9 qa-orchestrator-v3) + judgment-day = 17 files; the 11 sdd-* phase
        # skills need the SDD component too.
        # #4669: contributor workflow skills are not part of this preset.
        assert_file_count "$skills_dir" "SKILL.md" 17 "Ecosystem preset (skills alone): 17 skill files"
...
test_oc_skills_full() {
    log_test "OpenCode: skills injection (full-gentleman = 16 foundation skills)"
    cleanup_test_env

    # #3554: skills alone no longer pulls sdd. 16 foundation (7 original + 9
    # qa-orchestrator-v3) + judgment-day = 17.
    # #4669: the six contributor workflow skills are selectable, never default.
    if $BINARY install --agent opencode --component skills --preset full-gentleman --persona neutral 2>&1; then
        local skill_dir="$HOME/.config/opencode/skills"
        assert_dir_exists "$skill_dir" "OpenCode skill directory"
        assert_file_count "$skill_dir" "SKILL.md" 17 "Full preset (skills alone): 17 skill files"
